...
import { startPipelineWatch } from './pipeline-watch.ts'
import { cleanTool, type DshToolContext, type FollowupContext, type GitLabServiceLike } from './tools.ts'
 ...
 */
export function registerGitLabTools(ctx: DshToolContext & Partial<FollowupContext>, gitlab: GitLabServiceLike): void {
  ...
  register(cleanTool(defineTool({
    ...
    parameters: {
      ...
      source_branch: { type: 'string', description: 'Source branch name', required: true },
      target_branch: { type: 'string', description: 'Target branch name', required: true },
      title: { type: 'string', description: 'MR title', required: true },
      description: { type: 'string', description: 'MR description (markdown)' },
      reviewers: { type: 'array', description: 'Reviewer usernames (optional)', items: { type: 'string' } },
      watch_pipeline: { type: 'boolean', description: 'Auto-start pipeline watch (default: true)' },
    },
    output: {
    ...
    async execute(args) {
      ...
      const mr = await gitlab.createMR(
        ...
        Array.isArray(args.reviewers) && args.reviewers.length
          ? args.reviewers.map(String).join(', ')
          : undefined,
      )
  ...
  register(cleanTool(defineTool({
    ...
    parameters: {
      project: { type: 'string', description: 'Project ID (optional, uses default)' },
      mr_iid: { type: 'number', description: 'MR iid (the number after !)', required: true },
      action: {
        type: 'string',
    ...
    async execute(args) {
      ...
      switch (action) {
        case 'approve':
          ...
          return { ok: true, action: 'approved', mr_iid: mrIid }
        case 'request_changes':
          await gitlab.requestChanges(project, mrIid, comment ?? '')
          return { ok: true, action: 'changes_requested', mr_iid: mrIid }
        case 'comment':
          await gitlab.commentMR(project, mrIid, comment ?? '')
          return { ok: true, action: 'commented', mr_iid: mrIid }
        default:
  ...
  register(cleanTool(defineTool({
    ...
    parameters: {
      project: { type: 'string', description: 'Project ID (optional, uses default)' },
      name: { type: 'string', description: 'Tag name (e.g., v1.0.0)', required: true },
      ref: { type: 'string', description: 'Branch/commit to tag (defaults to default branch)' },
    ...
    async execute(args) {
      ...
      return { name: tag.name, commit_id: tag.commitId, message: tag.message }
    },
  })))
  ...
  register(cleanTool(defineTool({
    ...
    parameters: {
      project: { type: 'string', description: 'Project ID (optional, uses default)' },
      ref: { type: 'string', description: 'Branch/tag to check (defaults to default branch)' },
    },
    output: {
      schema: { type: 'object', additionalProperties: true },
      render: (_args, value) => renderObject(value),
    ...
    async execute(args): Promise<any> {
      ...
      if (!pipeline) {
        return { status: 'none', message: `No pipeline found for ref "${ref ?? 'default'}"` }
      }
      return {
        id: pipeline.id,
  ...
  register(cleanTool(defineTool({
    ...
    parameters: {
      project: { type: 'string', description: 'Project ID (optional, uses default)' },
      pipeline_id: { type: 'number', description: 'Pipeline ID', required: true },
    },
    output: {
      schema: { type: 'object', additionalProperties: true },
      render: (_args, value) => renderObject(value),
  ...
  register(cleanTool(defineTool({
    ...
    parameters: {
      ...
      branch: { type: 'string', description: 'Branch to monitor', required: true },
    },
    output: {
      schema: { type: 'object', additionalProperties: true },
      render: (_args, value) => renderObject(value),
    ...
    async execute(args) {
      ...
      switch (args.action) {
        case 'start':
          ...
          return { ok: true, message: `📡 Watching pipeline for branch: ${branch}` }
        case 'status':
          return { ok: true, message: `⏳ Pipeline watch for ${branch} is active (polling every 30s)` }
        default:
